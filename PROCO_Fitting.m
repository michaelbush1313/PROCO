...
h = msgbox('Click, hold and drag an ROI over the left ventricle, then double-click');
uiwait(h);
[Icrop_sax, rect_sax] = imcrop(imagesc(abs(squeeze(Bsax(:,:,refsax))),[0, max(abs(Bsax(:)))]));
h = msgbox('Click, hold and drag an ROI over a mid-ventricular slice, then double-click');
uiwait(h);
...
fclose(fileID);
type motion_correction_coeffs.txt
